/* {"name":"Volume Scroll","description":"Enable mousewheel volume control (v0.3.3)","author":"Araxeus","version":"0.3.3","hash":"aacd05f6c81e1e2e3e161605efc403de"} */
import{actions as l,intercept as k,store as j}from"@neptune";import{html as M}from"@neptune/voby";import{storage as s}from"@plugin";var $=(t)=>{s.settings??={};for(let r of Object.keys(t))s.settings[r]??=t[r];return s.settings};import{html as d,$ as w}from"@neptune/voby";import{html as h}from"@neptune/voby";var T=({children:t,tooltip:r})=>h`
	<div style="margin-bottom: 15px;display: flex;justify-content: space-between;align-items: center;" title="${r}">${t}</div>
`;var f=({value:t,onValue:r,title:o,tooltip:c})=>{const p=w(t),x=(i)=>{const g=i.target.value;if(!Number.isFinite(+g)||parseFloat(i.target.value)<1)i.target.value="1",p(1);else if(parseFloat(i.target.value)>20)i.target.value="20",p(20);r?.(i.target.value)},u=(i)=>{i.preventDefault();const g=i.deltaY<0?1:-1,v=Math.min(20,Math.max(1,p()+g));p(v),r?.(v.toString())};return d`
		<${T} tooltip=${c}>
